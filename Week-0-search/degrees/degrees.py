...
def load_data(directory):
    ...
    # Load the movie stars
    with open("movie stars.csv", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            try:
...
def shortest_path(source, target):
    """
    Returns to shortest list of (movie_id, person_id) pairs
    that connect the source to the target.

    If no possible path, returns None.
    """
    # TODO
    #  track of number of states explored
    num_explored = 0
    ...
    # Keep on looping until the solution found
    while True:
